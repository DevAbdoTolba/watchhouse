...
- [x] **ING-07**: User's main-stream captures are opened on demand for a single frame on trigger, then released — never held open continuously — with a short (~2 s) TTL cache for burst triggers on the same camera
- [x] **ING-08**: User's concurrent main-stream opens are serialized via a single global semaphore so DVR connection caps are not exceeded
